...
async def get_chart_data(
    ...
    try:
        ...
        db = get_firestore_client()
        if not db:
            raise HTTPException(status_code=503, detail="Service Unavailable: Firestore client not initialized")
...
async def _fetch_from_bigquery(symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not client:
            raise HTTPException(status_code=503, detail="Service Unavailable: BigQuery client not initialized")
