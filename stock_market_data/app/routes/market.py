...
def get_bigquery_client():
    """Get BigQuery client, initializing it only when needed"""
    try:
...
async def get_live_market_data(
    ...
    try:
        ...
        if symbols:
            ...
            # Use default symbols from Alpha Vantage service
            symbol_list = market_data_service.default_symbols[:5]  # Limit to 5 for rate limiting
        ...
        for symbol in symbol_list:
            try:
                ...
                if quote:
                    ...
                    # Try to get from Firestore cache as fallback
                    ...
                    if db:
                        try:
                            ...
                            logger.warning(f"Failed to get from Firestore cache: {e}")
                            market_data[symbol] = {
                                'symbol': symbol,
                                'error': 'No data available',
                                'status': 'data_unavailable'
...
async def get_single_quote(
    ...
    try:
        ...
        logger.error(f"Single quote fetch error: {e}")
...
async def get_chart_data(
    ...
    try:
        ...
        # Try to get data from Alpha Vantage based on period
        if period in ["1d", "5d"]:
            # Get intraday data for short periods
            intraday_data = await market_data_service.get_intraday_data(symbol_upper)
            if intraday_data:
                chart_data = [
                    {
            ...
            daily_data = await market_data_service.get_daily_data(symbol_upper)
            if daily_data:
                chart_data = [
                    {
        ...
        # If no Alpha Vantage data, try Firestore/BigQuery as fallback
        if not chart_data:
            chart_data = await _fetch_from_bigquery(symbol_upper, period)
...
@router.post("/refresh", response_model=MarketDataResponse)
async def refresh_market_data(
    ...
    """
    Refresh market data for specified symbols and publish to Pub/Sub
    """
    try:
        user_id = current_user.get("userId")
        if not user_id:
        ...
        if symbols:
            ...
            symbol_list = market_data_service.default_symbols
        
        # Start background task to fetch and publish data
        if background_tasks:
            background_tasks.add_task(market_data_service.fetch_and_publish_quotes, symbol_list)
        
        return MarketDataResponse(
            success=True,
            data={'symbols': symbol_list},
            message=f"Started refresh for {len(symbol_list)} symbols"
        )
...
@router.get("/summary", response_model=MarketDataResponse)
async def get_market_summary(
    ...
    try:
        ...
        if symbols:
            ...
            symbol_list = None  # Use default symbols
        
        # Get market summary
        summary = await market_data_service.get_market_summary(symbol_list)
        
        return MarketDataResponse(
            success=True,
            data=summary,
            message="Retrieved market summary"
        )
...
@router.get("/suggestions", response_model=MarketDataResponse)
async def get_symbol_suggestions(
    query: str = Query(..., description="Search query for symbols"),
    current_user: dict = Depends(lambda: {})
):
    """
    Get symbol suggestions based on search query
    """
    try:
        user_id = current_user.get("userId")
        if not user_id:
        ...
        suggestions = market_data_service.get_symbol_suggestions(query)
        
        return MarketDataResponse(
            success=True,
            data={'suggestions': suggestions},
            message=f"Found {len(suggestions)} symbol suggestions"
        )
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Symbol suggestions error: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch symbol suggestions")

async def _fetch_from_bigquery(symbol: str, period: str) -> List[Dict[str, Any]]:
    """Fetch historical data from BigQuery"""
    try:
        client = get_bigquery_client()
        if not client:
            logger.warning("BigQuery client not initialized, cannot fetch data.")
            return []
        
        # Determine table and date range based on period
        if period in ["1d", "5d"]:
            table_name = settings.BIGQUERY_INTRADAY_TABLE
            start_date = datetime.utcnow() - timedelta(days=1) # Default to 1 day for intraday
        elif period in ["1m", "3m", "6m"]:
            table_name = settings.BIGQUERY_DAILY_TABLE
            start_date = datetime.utcnow() - timedelta(days=30) # Default to 30 days for daily
        elif period == "1y":
            table_name = settings.BIGQUERY_DAILY_TABLE
        ...
        WHERE symbol = '{symbol}'
        AND timestamp BETWEEN '{start_date.isoformat()}' AND '{datetime.utcnow().isoformat()}'
        ORDER BY timestamp
        """
