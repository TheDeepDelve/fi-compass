...
class MCPClient:
    ...
    async def login(self, phone_number: str) -> Dict[str, Any]:
        """
        Login to Fi MCP server with phone number using form data as required by backend
        """
        try:
            # The sessionId is required by the backend, but for login we can send a dummy or empty value
            form_data = {
                "sessionId": "",  # or generate a UUID if needed
